...
"""
    ...
    # index.html
    final_index = get_full_page(article_content, latest_4, True)
    with open("index.html", "w", encoding="utf-8") as f:
        f.write(final_index.replace("{{latest_cards_placeholder}}", latest_4))

    # archive.html
    all_cards = gen_cards(posts, is_root=True)
    archive_body = f"""
    <div class="py-12">
    ...
    with open("archive.html", "w", encoding="utf-8") as f:
        f.write(get_full_page(archive_body.replace("{{all_cards}}", all_cards), "", True))

    # archive/ 個別記事
    latest_4_rel = gen_cards(posts[1:5], is_root=False)
    final_article = get_full_page(article_content, latest_4_rel, False)
    with open(f"archive/{{new_slug}}.html", "w", encoding="utf-8") as f:
        f.write(final_article.replace("{{latest_cards_placeholder}}", latest_4_rel))
