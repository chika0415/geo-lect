...
{{
  ...
  "slug": "タイトルを英語の小文字とハイフンのみで表現 (例: oil-crisis-vietnam)",
  "image_query": "Unsplashで検索するための英語キーワード1語",
...
try:
    ...
    new_post = {
        ...
        "img": f"https://source.unsplash.com/featured/800x450?{data['image_query']}"
    }
    posts.insert(0, new_post) # 先頭に追加
    posts = posts[:20] # 最大20件保存
    ...
    # --- 5. アーカイブUIの生成 ---
    archive_cards_html = ""
    for p in posts[1:10]: # 最新以外をアーカイブに表示
        archive_cards_html += f"""
        <a href="../{p['url'] if 'archive/' in p['url'] else p['url']}" class="group block card rounded-2xl overflow-hidden hover:border-red-600 transition shadow-sm">
    ...
    segments_html = "".join([f'<div class="grid md:grid-cols-2 gap-8 border-b border-slate-200 pb-8"><div class="text-xl serif leading-relaxed">{s["en"]}</div><div class="text-lg opacity-60 italic border-l-4 border-red-600/30 pl-4">{s["jp"]}</div></div>' for s in data['segments']])
...
<head>
    ...
    <meta name="description" content="{data['seo_description']}"><meta name="keywords" content="{data['seo_keywords']}">
...
<body class="antialiased">
    ...
    <main class="max-w-7xl mx-auto px-6 py-12">
        ...
        <img src="{new_post['img']}" class="w-full h-64 md:h-96 object-cover rounded-3xl mb-16 shadow-2xl">
        <div class="grid lg:grid-cols-4 gap-12">
            <div class="lg:col-span-3 space-y-12">
                {segments_html}
                <div class="card rounded-3xl p-8 border-l-8 border-blue-600 bg-blue-600/5">
                    <h4 class="font-bold mb-2 text-blue-700">🤖 Intelligence Insight</h4>
    ...
    sitemap += f'  <url><loc>https://chika0415.
