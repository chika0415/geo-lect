...
# --- 3. 記事生成 ---
prompt = f"""
最新の地政学リスクから投資家が注目すべきトピックを1つ選び、英語学習教材を作成してください。
以下のJSON形式で厳密に出力してください：
{{
  "en_title": "英語の見出し",
  "jp_title": "日本語の見出し",
  "seo_keywords": "投資, 地政学, [キーワード3つ]",
...
try:
    ...
    archive_cards_html = ""
    for p in posts[1:10]:
        # index.htmlからの相対パスにするための調整
        p_url = p['url']
        archive_cards_html += f"""
        <a href="{{p_url}}" class="group block card rounded-2xl overflow-hidden hover:border-red-600 transition shadow-sm">
...
<body class="antialiased">
    ...
    <main class="max-w-7xl mx-auto px-6 py-12">
        ...
        <img src="{{img_url}}" class="w-full h-64 md:h-96 object-cover rounded-3xl mb-16 shadow-2xl">
        <div class="grid lg:grid-cols-4 gap-12">
            <div class="lg:col-span-3 space-y-12">
                {{segments_html}}
                <div class="card rounded-3xl p-8 border-l-8 border-blue-600 bg-blue-600/5">
                    <h4 class="font-bold mb-2 text-blue-700 font-bold italic">🤖 Gemini's Strategy Insight</h4>
